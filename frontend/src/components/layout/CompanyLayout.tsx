...
import { COMPANY_NAVIGATION } from '../../constants';
import {
  LayoutDashboard,
  Briefcase,
  Users,
...
function CompanyLayout() {
  ...
  const isActive = (path: string) => {
    return location.pathname === path || (path !== '/company' && location.pathname.startsWith(path));
  };

  const getPageTitle = () => {
    const currentPath = location.pathname;
    const navItem = COMPANY_NAVIGATION.find(
      (item) => item.path === currentPath || (item.path !== '/company' && currentPath.startsWith(item.path))
    );
    return navItem?.label || 'RECRUITER DASHBOARD';
  };
    ...
    <div className="flex h-screen bg-[#0A0C10] text-[#E7EAF0] overflow-hidden px-grid-bg">
      ...
      >
        <div className="flex h-16 items-center justify-between px-5 border-b border-[#262B38] bg-[#0A0C10]/60">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="font-mono text-[9px] block text-[#F2A93B] tracking-wider uppercase">COMPANY PORTAL</span>
            </div>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        <nav className="flex-1 space-y-1 px-2">
          {COMPANY_NAVIGATION.map((item, idx) => {
        ...
        <header className="h-16 border-b border-[#262B38] bg-[#12151C] px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex items-center gap-2 px-2.5 py-1 bg-[#0A0C10] border border-[#262B38] font-mono text-[11px]">
              ...
              <span className="text-[#34D399]">PORTAL ACTIVE</span>
            </div>
